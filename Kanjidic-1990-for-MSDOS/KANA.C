...
int _kana=1; /* flag to tell whether text is converted to kana */


int getjis(secondkana,rest,string)
int *secondkana;
char **rest;
char *string;
{
        ...
        *secondkana = NOTHING;
        ...
        /* If not _kana flag on or not letter, then convert it directly: */
        if(!_kana || !isalpha(*string))
         {
           *rest = (string+1);
           *secondkana = 0;
           return(asctojis(*string));
         }

        result = getkana(secondkana,rest,string);
        if(!isalpha(*(string-1)) && !isalpha(**rest))
         { /* If this kana is not part of any word, then it may be particle: */
...
int getkana(secondkana,rest,string)
int *secondkana;
char **rest; /* of string */
char *string;
{
